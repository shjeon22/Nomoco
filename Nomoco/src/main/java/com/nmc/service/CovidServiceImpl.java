...
public class CovidServiceImpl implements CovidService {
	...
	public JSONArray getCovidInfo() {
		...
		Elements covid_num0 = doc
				.select("#content > div > div.caseTable > div:nth-child(1) > ul > li:nth-child(1) > dl > dd"); // 2)
																												// 사망수1
		Elements covid_num1 = doc
				.select("#content > div > div.caseTable > div:nth-child(1) > ul > li:nth-child(2) > dl > dd"); // 3)
																												// 사망수2
		Elements covid_num2 = doc
				.select("#content > div > div.caseTable > div:nth-child(2) > ul > li:nth-child(1) > dl > dd"); // 4) 재원
																												// 위중증1
		Elements covid_num3 = doc
				.select("#content > div > div.caseTable > div:nth-child(2) > ul > li:nth-child(2) > dl > dd"); // 5) 재원
																												// 위중증2
		Elements covid_num4 = doc
				.select("#content > div > div.caseTable > div:nth-child(3) > ul > li:nth-child(1) > dl > dd"); // 6)
																												// 신규입원1
		Elements covid_num5 = doc
				.select("#content > div > div.caseTable > div:nth-child(3) > ul > li:nth-child(2) > dl > dd"); // 7)
																												// 신규입원2
		Elements covid_num6 = doc
				.select("#content > div > div.caseTable > div:nth-child(4) > ul > li:nth-child(1) > dl > dd"); // 8) 확진1
		Elements covid_num7 = doc
				.select("#content > div > div.caseTable > div:nth-child(4) > ul > li:nth-child(2) > dl > dd"); // 9) 확진2
...
//					log.info(obj+"");
	...
	public JSONArray getCovidinfo3() {
		// 코로나 FAO(미사용 X)
		String url = "http://ncov.mohw.go.kr/faqBoardList.do?brdId=3&brdGubun=38";
		...
		Elements f_title1 = doc.select("#content > div > div:nth-child(3) > ul > li.open > a > span.fl_ttl"); // 1.FAQ(1)
																												// title
		Elements f_answer1 = doc.select("#\\33 473"); // 2. FAQ(1)-answer
		Elements f_title2 = doc.select("#content > div > div:nth-child(9) > ul > li.open > a > span.fl_ttl"); // 3.FAQ(2)
																												// title
		Elements f_answer2 = doc.select("#\\33 500p"); // 4.FAQ(2)-answer
		Elements f_title3 = doc.select("#content > div > div:nth-child(11) > ul > li:nth-child(4) > a > span.fl_ttl"); // 5.FAQ(3)-title
		Elements f_answer3 = doc.select("#\\33 511p"); // 6.FAQ(4)-answer

		JSONArray covidList3 = new JSONArray();

		for (int i = 0; i < f_title1.size(); i++) {
			JSONObject obj = new JSONObject();

			obj.put("f_title1", f_title1.get(i).text());
			obj.put("f_answer1", f_answer1.get(i).text());
			obj.put("f_title2", f_title2.get(i).text());
			obj.put("f_answer2", f_answer2.get(i).text());
			obj.put("f_title3", f_title3.get(i).text());
			obj.put("f_answer3", f_answer3.get(i).text());

			covidList3.add(obj);
			System.out.println("covidList3" + covidList3);
